...
def get_all_tags(client, args, image_dict_list):
    screen_layout = None
    if args.screen:
    ...
    print("Finished get_all_tags:\n{}".format(image_dict_list))
    return image_dict_list
...
def get_tags(image, shared_data):
    ...
    # print("OMG: {} - {}".format(image['name'], response))
    for tag in response['results']:
        ...
        logger.debug('Tag {} Processed'.format(image['name']))
    print("exiting: {} ** {} ".format(image['name'], image_tags_dict_list))
